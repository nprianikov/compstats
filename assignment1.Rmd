...
The $\text{Var}(e)$ depends on the the sum of squared residuals due the variance being defined as $\frac{1}{n} \sum_i {e_i^2}$.

Claim: the sum of squared residuals can either decrease or stay the same when adding parameters to the models. To illustrate this we define a simple model $Y = \alpha + X^{(1)} \beta_1 + \epsilon$ with a single parameter, and $Y' = \alpha + X^{(1)} \beta_1 + X^{(2)} \beta_2 + \epsilon$ that extend first model with the second parameter. The residuals are defined respectively as
...
Due to the fact that OLS minimizes the criterion $\sum_i {e_i^2}$ , it aims to find the set of parameters that minimizes the total squared residuals. In the model with a single parameter, OLS will find an optimal $\beta_1$ that minimizes the sum of squared residuals given the constraint of the model structure. Adding $\beta_2$ allows the model to potentially account for more variance in $Y$ that $X^{(1)}$ alone could not explain.The parameter $\beta_2$ will take on a value that contributes to either a reduction in the total sum of squared residuals or leaves it unchanged. The latter scenario occurs when $X^{(2)}$ does not provide any additional explanatory power over what is already explained by $X^{(2)}$. However, if $X^{(2)}$ does explain additional variance in $Y$, then the sum of squared residuals will decrease.
...
= 1 - \frac{\sigma^2}{\text{Var}(Y)} \times \frac{n - 1}{n - p - 1}
$$
