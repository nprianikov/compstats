...
simulate_R_squared <- function(n, p, m) {
  ...
  # Step 2 & 3: Extend X with m additional predictors
  X_new <- matrix(rnorm(m * n), nrow = n, ncol = m)
  X_extended <- cbind(X, X_new)
  ...
  r_squared_extended <- summary(model_extended)$r.squared
  
  return(r_squared_extended)
}
...
monte_carlo_simulation <- function(n, p, r, m) {
  # Replicate r times the simulation
  simulations <- replicate(r, simulate_R_squared(n, p, m))
  
  # Compute the mean R^2 over all simulations
  mean_R_squared <- mean(simulations)
  return(mean_R_squared)
}
```
...
Further, 
$$
\text{Var}(e) = \sigma^2 = E(\hat{\sigma}^2 | X) = \frac{1}{n-p} \sum_i {e_i^2}.
$$
Thus,
$$
R^2 = 1 - \frac{\text{Var}(e)}{\text{Var}(Y)}
    = 1 - \frac{\sigma^2}{\text{Var}(Y)} 
    = 1 - \frac{E(\hat{\sigma}^2 | X)}{\text{Var}(Y)}
    = 1 - \frac{\frac{1}{n-p} \sum_i {e_i^2}}{\text{Var}(Y)}
$$
Hence, as $p \rightarrow \infty$, $\frac{1}{n-p}$ goes to $0$, bringing the right part of the equation to $0$ and thus $R^2$ to $1$.
